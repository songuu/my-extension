import { CONTEXT_MENU, COPY, KEY_DOWN, PAGE_LOADED, SELECT_START } from "./constant";
...
const handleInsert = () => {
  ...
  const originObject = {
    context2DPrototype: (window as any)?.document
  ...
  console.log('canvasDataGroup', canvasDataGroup)
  ...
  const render = () => {
    ...
    const templateText: string[] = canvasDataGroup.map((canvasData, index) => {
      ...
      const textItem: string[] = canvasData.data.map(
        (item) =>
          `<div style="position: absolute; left: ${item[1]}px; top: ${item[2] + computedTop
          }px">${item[0]}</div>`
      )
...
const enableUserSelectByCSS = (): void => {
...
const enableOnCopyByCapture = (): void => {
  window.addEventListener("copy", stopNativePropagation, true);
  document.addEventListener("copy", stopNativePropagation, true);
}

export { handleInsert, enableUserSelectByCSS, enableOnCopyByCapture, hideButton }